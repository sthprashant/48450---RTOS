 ...
 *Student ID: 11823871
 *Date: 29/05/2020
 *Topic: Simulation of page-replacement for virtual memory management by using First-In-First-Out (FIFO) Algorithm
*/
...
int IndexMax(uint num[]);
int Frame(uint bulk[], int check);
void PrintFrame(uint frame[]);
...
{
	// Asks for a valid argument if the number of frames is not entered
	if (argc != 2)
	{
	...
	// Asks for a valid argument greater than 0
...
/**
 ...
 @param num stores number of frames
...
void SignalHandler(int signal)
{
	// Prints the toal page faults once user Ctrl+c and exits the program
	if (signal == SIGINT)
	{
		printf("\nTotal page faults: %d\n", pageFaults);
		exit(0);
